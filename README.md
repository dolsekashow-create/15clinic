...
```
apps/web            Next.js — Dashboard + API v1
packages/core       الكيانات، Zod schemas، كتالوج الصلاحيات، الأخطاء  (بدون تبعيات)
packages/data       Repositories — نقطة فرض عزل الفروع الوحيدة
packages/services   منطق العمل: الحضور، المحاسبة، سجل العمليات
packages/auth       الجلسة، حل الصلاحيات، withAuth guard
packages/infra      Firebase Admin، اللوج، مزوّدو الإشعارات
tools/seed          بيانات التطوير (isDemo: true)
```

قواعد الاستيراد بين الطبقات تُفرض بـESLint: الواجهة لا تستطيع استيراد طبقة البيانات، وطبقة الخدمات لا تعرف Firestore.
...
pnpm seed                      # ينشئ المؤسسة والفروع والصلاحيات وشجرة الحسابات
pnpm dev
```
