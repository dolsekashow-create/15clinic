...
> **تحديث مهم:** المشروع بقى **تطبيق Next.js واحد** في جذر المستودع بدل pnpm monorepo.
> كل الأخطاء اللي تحت (`ERR_PNPM_META_FETCH_FAIL`, `next: command not found`,
> `No Next.js version detected`) كان سببها المشترك هو الـworkspaces، ومبقاش لها وجود.
> على Vercel: Root Directory = `./`، مفيش install/build command مخصص، مفيش `vercel.json`.
> الأقسام التالية محفوظة للرجوع إليها لو رجعت لبنية monorepo مستقبلًا.
